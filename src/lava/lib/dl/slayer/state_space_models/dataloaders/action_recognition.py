...
class NTUEfficientNetDataset(torch.utils.data.Dataset):
    r"""
    ...
    def balance_by_random_drop(self, force_min=None):
        ...
        # num_samples_to_drop = class_counts - min_class_count
        ...
        for class_id, to_drop in enumerate(num_samples_to_drop):
            ...
            self.video_list = [label for id, label in enumerate(self.video_list) if id not in ids_to_drop]

    def _load_image(self, base_path: str, frame_index: list) -> Image.Image:
        data = np.load(base_path)

        return data[np.array(frame_index)]



    def _parse_annotationfile(self):
        self.video_list = [VideoRecord(x.strip().split(), self.root_path) for x in open(self.annotationfile_path)]
        self.label_list = [self.label_map.get(record.label, 0) for record in self.video_list]
    ...
    def _sanity_check_samples(self):
        ...
        for i, record in enumerate(self.video_list):
            if record.num_frames <= 0 or record.start_frame == record.end_frame:
                ...
                #      f"={self.num_segments * self.frames_per_segment} frames. Dataloader will repeat the "
                #      f"first frame when trying to load this video.\n")
        print(f"encountered {len(warning_idx)=}. Deleting those from file list.")

        for i in reversed(warning_idx):
            del self.video_list[i]
            del self.label_list[i]
        print("num labels after removal:", len(self.video_list), len(self.label_list))
    ...
    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        ...
        frame_idx = []
        for idx in frame_start_indices:
            idx = int(idx)
            frame_idx.append(idx)

            # load self.frames_per_segment consecutive frames
            for _ in range(self.frames_per_segment - 1):

                if idx < record.end_frame:
                    idx += 1
                    frame_idx.append(idx)


        images = self._load_image(record.path, frame_idx) 
...
def init_effnet_NTU_dataloader(partition="train",
                               batch_size=8,
                               num_frames_per_sample=30,
                               data_root=None):
    # videos_root = os.path.join("/ssd2/users/pweidel/datasets/NTU/data_frames/")
    annotation_file = os.path.join(data_root, partition + '_effnet_features.txt')
    ...
    dataset = NTUEfficientNetDataset(
        root_path="",
