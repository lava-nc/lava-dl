    ...
    if pruning_method=='random':
        ...
        for k,iw in enumerate(index_weights):
            ...
            for kk in range(index_weights[k-1]+1 if k>0 else 1, iw+1):
                ...
                check = (S[0]>1)
                check = 0 if (kk==0 or kk == len(MM)-1 or 'head1_blocks.1.weight' in [*MM.keys()][kk]) else check
                if check:
...
def networkPruning_(saved_net_path, saved_to_net_path = None, pruning_ratio = 2, pruning_method = "random", log_level = 0):
    ...
    for k, param in enumerate(module_short.state_dict()):    
        ...
        check = (S[0]>1)
        check = 0 if (k==0 or k == len(module_short.state_dict())-1 or param=='head1_blocks.1.weight') else check
    ...
    if pruning_method in ('L1', 'L2'):
        ...
        for k,iw in enumerate(index_weights):
            ...
            for kk in range(index_weights[k-1]+1 if k>0 else 1, iw+1):
                ...
                check = (S[0]>1)
                check = 0 if (kk==0 or kk == len(nameList)-1 or 'head1_blocks.1.weight' in nameList[kk]) else check
                if check:
...
def toPanda(arg_list, update_flag = True, export_to_pkl = True, export_to_excel=False):
    ...
    indx = [aa.split(' ')[-1][-13:-1] for aa in arg_list if aa.split(' ')[0]=='output_path']
    c_.index = indx
    
    if not (indx in list(Collection.index)) or update_flag:
        Collection = Collection.drop(index=indx) if indx in list(Collection.index) else Collection
        Collection = pd.concat([Collection, c_])
        storeData.save(Collection,'trained_models_collection.pkl') if export_to_pkl else None
        Collection.to_excel("trained_models_collection.xlsx") if export_to_excel else None
    return Collection
...
def retrieve_path(seedStr, verbose = False, search_pos = -1, removePaths = False, confirm_deletion = False):
    ...
    SEARCH_STR = '**/' + ('%s*/**/' if search_pos==1 else '') + ('*%s*/**/' if search_pos==0 else '') + ('*%s' if search_pos==-1 else '')
    ...
    print('ATTENTION: more than 1 entry for final search string pattern: %s'%seedStr) if len(G_extras)>2 else None
    if G_extras==[]:
        print('FAILED: Trained folder not found or wrong search string') if verbose else None
        return []
...
def retrieve_info(seedStr, verbose=False, track_loaded_models = False, confirm_deletion = False):
    ...
    try:
        ...
        retrieve_path(seedStr, removePaths = True, confirm_deletion = confirm_deletion);
        print('=============>|Terminating')
        return None  
    ...
    if verbose:        
        args2print = {'DVSlike', 'Heads', 'b', 'load', 'model', 'lr', 'aug_prob', 'sparsity', 'threshold'}
        ...
        print('**** Further info ****')
        print('- Full network path: %s'%'/'.join(dataAT.split('/')[:-1])+'/network.pt')
        print('- Args: ', *[a[:-1] for a in RLargs if a.split(' : ')[0] in args2print], 
              sep = ' | ')
        if track_loaded_models:
            loaded_models = []
            loaded_models.append(os.path.dirname(extractPath(extractValue('load', RLargs))))
            accuracy = []
            while loaded_models[-1] is not None:
                ...
                if RLargs_ != None:
                    RLargs_ = RLargs_[2]
                    loaded_models.append(os.path.dirname(extractPath(extractValue('load', RLargs_))))
            ...
            print(*[(l+': '+a) for l,a in zip(loaded_models[::-1], accuracy[::-1])], sep=' \-->\n')              
...
if __name__ == '__main__':
    ...
    parser.add_argument('-output_dir', type=str,   default="Combine_heads_KP",    help="folder in which we store predictions+bboxes")
    parser.add_argument('-models', type=str, default='Full', help='[Full, Half]')
    ...
    parser.add_argument('-search_str', type=str, default=[], help='search strings based on last patterns on the run', nargs='+')
    parser.add_argument('-inference', default=False, action='store_true', help='run inference')
    parser.add_argument('-y', default=False, action='store_true', help='confirm deletions')
    ...
    G = [glob(f'{args.output_dir}/%s/head{k}_*'%'_'.join(network_path[k].split('/')[-2].split('_')[1:])) for k in range(n_heads)]
    ...
    def combined_accuracy(conf_threshold):
        ...
        for pred, bb in zip(predTot, BB):
            T = pred.shape[-1]
            det = [nms(pred[...,t], conf_threshold=conf_threshold) for t in range(T)]
    ...
    Gperm_ind += [comb for comb in itertools.combinations(range(0,n_heads), 2)]
    for kG in Gperm_ind: 
