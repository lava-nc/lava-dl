...
COCO_path    = ['/export/share/datasets/COCO2017', '/data-raid/sshresth/data/coco/']
BDD100K_path = ['/export/share/datasets/BDD100K/MOT2020/bdd100k', '/data-raid/sshresth/data/bdd100k/MOT2020/bdd100k']
...
if __name__ == '__main__':
    ...
    # Pretrained model
    parser.add_argument('-load'      , type=str, default='',  help='pretrained model', nargs='+')
    # Target generation
    ...
    # dataset
    parser.add_argument('-dataset'        , type=str,   default='COCO',help='dataset to use [COCO, BDD100K, PASCAL]')
    parser.add_argument('-path'           , type=str,   default=None,  help='dataset path') #this one will be set to 
    ...
    parser.add_argument('-train'          , default=False, action='store_true', help='activate training')
    parser.add_argument('-print_summary'  , default=False, action='store_true', help='Print model summary and exit')
    parser.add_argument('-strID', type=str, default='SDNN_',                    help='str ID to attach to file name')
    parser.add_argument('-DVSlike'        , default=False, action='store_true', help='activate random image jitter subtraction')
    parser.add_argument('-model', type=str, default='single_head_KP',           help='(list of) model(-heads) to load [dual_head, single_head_KP, short_single_head_KP, ...]', nargs='+')
    parser.add_argument('-Heads', type=int, default=None,                       help="[list of] head id", nargs ='+')
    ...
    if args.combined_models>1: ### multiple heads loading the combined head model
        ...
        exec('from models.sdnn_%s import Network, SparsityMonitor'%args.model[0])
    
    #### selecting path according to its existance - see above for including another server path    
    DATASET_path = COCO_path if args.dataset=='COCO' else BDD100K_path
    args.path = [g for g in DATASET_path if os.path.exists(g)][0] if not args.path else args.path
    print(f'loading dataset from {args.path}')
    
    if args.train:
        from datetime import datetime
        date_str = str(datetime.now())[:-9].replace(':', '').replace('-', '').replace('.', '').replace(' ', '')
    ...
    if args.sparsity:
        ...
        sparsity_monitor = SparsityMonitor(max_rate=args.sp_rate, lam=args.sp_lam)
    else:
    ...
    if args.dataset == 'COCO':        
        if args.subset:
            ...
            if args.train:
                train_set = COCO(root=args.path, train=True, augment_prob=args.aug_prob, image_jitter = args.DVSlike)            
                train_loader = DataLoader(train_set,
                                        batch_size=args.b,
                                        shuffle=True,
                                        collate_fn=yolo_target.collate_fn,
                                        num_workers=args.num_workers,
                                        pin_memory=True)
            test_set = COCO(root=args.path, train=False, image_jitter = args.DVSlike)
        ...
        if args.train:
            train_set = BDD(root=args.path, dataset='track', train=True, augment_prob=args.aug_prob, randomize_seq=True, image_jitter = args.DVSlike)                
            train_loader = DataLoader(train_set,
                                    batch_size=args.b,
                                    shuffle=True,
                                    collate_fn=yolo_target.collate_fn,
                                    num_workers=args.num_workers,
                                    pin_memory=True)
        test_set = BDD(root=args.path, dataset='track', train=False, randomize_seq=True, image_jitter = args.DVSlike)
        test_loader = DataLoader(test_set,
                                batch_size=args.b,
                                shuffle=False,
                                collate_fn=yolo_target.collate_fn,
    ...
    m_s = lambda x: '%.2f±%.2f'%(np.mean(x), np.std(x))
    rate_count_test = []


    (summary(module, (args.b, 3, 448, 448)), os.exit()) if args.print_summary else None
    ...
    for epoch in pbar_epoch:
        if args.train:
            ...
            # print(f'{epoch=}')
            pbar = tqdm(train_loader, desc='Train', leave=False, bar_format = "{desc}: {percentage:.1f}%|{bar}|{n_fmt}/{total_fmt} {postfix}")          
            for i, (inputs, targets, bboxes) in enumerate(pbar):
                ...
                H = '|Trn:%.3f'%stats.training.accuracy + ' |Rate:' + m_s([c.item() for c in counts[0]])
                H = H +'(Mx: %.3f)'%stats.training.max_accuracy if stats.training.best_accuracy else H
                try:
        ...
        pbar = tqdm(test_loader, desc='Test', leave=False, bar_format = "{desc}: {percentage:.1f}%|{bar}|{n_fmt}/{total_fmt} {postfix}")
        for i, (inputs, targets, bboxes) in enumerate(pbar):
            net.eval()
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    pass
                H = '|Tst:%.3f'%stats.testing.accuracy + (H if stats.training.accuracy else '') + ' |Rate:' + m_s([c.item() for c in counts[0]])
        ...
        if epoch == args.epoch-1:
            storeData.save(rate_count_test, args.output_dir+'/'+args.strID+"_rate_count_test.pkl")
