...
class TestHdf5Netx(unittest.TestCase):
    ...
    def test_recurrent_netx_save_and_load(self) -> None:
        ...
        class Network(torch.nn.Module):
            def __init__(self,):
                ...
                self.blocks = torch.nn.ModuleList([
                        slayer.block.cuba.Recurrent(self.cuba_params, 6, 5, weight_scale=0.0, pre_hook_fx=None),
                        slayer.block.cuba.Recurrent(self.cuba_params, 5, 5, weight_scale=0.0, pre_hook_fx=None),
                    ])
        ...
        eps = 0.02
        assert np.abs((lava_dl_output[0][0].mean() - lava_output[0].mean())/lava_dl_output[0][0].mean()) <= eps, f'Mean firing rate mismatch {lava_dl_output[0][0].mean()=}; lava mean firing rate {lava_output[0].mean()=}'
